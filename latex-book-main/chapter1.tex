% Chương 1: Bối cảnh, Vấn đề và Mục tiêu Đề tài
...
Hiện nay, phần lớn các đơn vị tổ chức cuộc thi nhiếp ảnh phim vẫn đang vận hành thông qua sự kết hợp của các công cụ thủ công và phân tán. Cụ thể, khâu thông báo thể lệ và tiếp nhận đơn đăng ký thường dựa vào các biểu mẫu trực tuyến tự tạo như Google Forms hay Microsoft Forms kết hợp bài đăng trên mạng xã hội Facebook và Instagram. Về mặt lưu trữ, tệp ảnh bài dự thi bị phân tán rải rác trên nhiều dịch vụ đám mây cá nhân như Google Drive, Dropbox, OneDrive hoặc tiếp nhận đính kèm qua thư điện tử. Bên cạnh đó, toàn bộ dữ liệu quản lý thí sinh, thông số kỹ thuật cuộn phim và kết quả thẩm định đều được tổng hợp thủ công trên các bảng tính Microsoft Excel hoặc Google Sheets. Đến giai đoạn đánh giá, ban tổ chức phải sao chép danh sách ảnh thành các tệp bảng tính rời rạc để gửi email cho từng giám khảo, sau đó tổng hợp điểm số bằng các công thức tính toán thủ công, dẫn đến nguy cơ sai lệch số liệu và tiêu tốn hàng trăm giờ lao động hành chính.
...
\begin{itemize}
    \item \textbf{BO-01 (Tập trung hóa)}: Đặc tả nền tảng số hóa đề xuất và xây dựng nền tảng dữ liệu thống nhất cho các nghiệp vụ quản lý cuộc thi nhiếp ảnh phim trong phạm vi đồ án cơ sở dữ liệu.
    ...
    \item \textbf{BO-05 (Thẩm định AI có Kiểm soát)}: Đặc tả ranh giới tích hợp AI và mô hình lưu kết quả phân tích advisory để hỗ trợ phát hiện ảnh trùng lặp và ảnh tạo bởi AI theo mô hình \textit{Human-in-the-loop}; việc huấn luyện và inference AI thực tế nằm ngoài phase này.
\end{itemize}
...
Để đảm bảo tính tập trung và chiều sâu học thuật của một đồ án Thiết kế Cơ sở Dữ liệu chuyên sâu, phạm vi dự án được phân định rạch ròi như thể hiện trong Bảng~\ref{tab:c1_scope}:
...
\label{tab:c1_scope}
\small
\begin{tabularx}{\textwidth}{p{3.0cm}p{6.0cm}>{\raggedright\arraybackslash}X}
\toprule
...
$\bullet$ Thiết kế kiến trúc hệ thống đề xuất (Context, Modules, AI Integration Boundary) \newline
...
$\bullet$ Lập trình giao diện Web và ứng dụng Mobile; đây là các client được đề xuất trong kiến trúc \newline
$\bullet$ Lập trình Backend API runtime; đây là service được đề xuất trong kiến trúc \newline
$\bullet$ Huấn luyện, hosting hoặc inference mô hình AI thực tế; AI service chỉ là thành phần tích hợp được đề xuất \newline
$\bullet$ Triển khai object storage đám mây thực tế; CSDL chỉ lưu URI/path, hash và metadata &
...
Nhằm định hình rõ ranh giới nghiên cứu và đảm bảo tính khả thi kỹ thuật cao nhất, thiết kế của đề tài được xây dựng trên các giả định cốt lõi: Trước hết, hệ quản trị cơ sở dữ liệu mục tiêu được chuẩn hóa duy nhất là Microsoft SQL Server 2022+ vận hành trên môi trường Linux Container. Mặt khác, nhằm bảo đảm tính nhân văn và khách quan trong đánh giá nghệ thuật, mọi kết quả xử lý từ trí tuệ nhân tạo (AI) đều được định vị mang tính chất tư vấn hỗ trợ quyết định (\textit{advisory}); qua đó hệ thống tuyệt đối không tự động loại bỏ bài thi nếu chưa có sự phê duyệt chính thức từ Ban tổ chức. Cuối cùng, kiến trúc giả định các tệp nhị phân hình ảnh scan dung lượng cao được lưu trữ tại dịch vụ lưu trữ đối tượng chuyên dụng (\textit{Object Storage}), do đó cơ sở dữ liệu chỉ tập trung quản lý đường dẫn URI tham chiếu, mã băm toàn vẹn SHA-256 cùng các siêu dữ liệu kỹ thuật có cấu trúc.
...
AI Analysis Result & Bản ghi kết quả phân tích tự động từ AI (độ tương đồng, xác suất ảnh AI, nhãn tự động) kèm độ tin cậy. \\
...
Kiến trúc đích phục vụ 4 nhóm tác nhân người dùng chính cùng các bên liên quan bên ngoài. Bảng~\ref{tab:c1_stakeholders} tổng hợp trách nhiệm và nhu cầu thông tin của từng tác nhân; các client Web/Mobile và Backend runtime để hiện thực tương tác này chưa thuộc deliverable của phase cơ sở dữ liệu:
...
\small
\begin{tabularx}{\textwidth}{p{2.8cm}p{4.2cm}>{\raggedright\arraybackslash}X}
\toprule
...
Hệ thống áp dụng mô hình phân quyền truy cập dựa trên vai trò (\textit{Role-Based Access Control - RBAC}), trong đó một tài khoản người dùng (\texttt{UserAccount}) có thể nắm giữ một hoặc nhiều vai trò (\texttt{Role}) đồng thời thông qua bảng kết hợp \texttt{UserRole}, cụ thể như một người dùng vừa là Thí sinh trong cuộc thi này, vừa có thể đảm nhiệm vai trò Giám khảo tại cuộc thi khác. Tuy nhiên, nhằm triệt tiêu hoàn toàn hiện tượng xung đột lợi ích (\textit{Conflict of Interest}), các hành vi nghiệp vụ trong từng ngữ cảnh cuộc thi phải tuân thủ nghiêm ngặt quy tắc: một người dùng đã đăng ký dự thi trong một cuộc thi cụ thể thì tuyệt đối không thể được phân công làm Giám khảo hoặc thành viên Ban tổ chức của chính cuộc thi đó.
