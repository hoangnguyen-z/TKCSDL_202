...
Báo cáo này được cấu trúc thành 5 phần, 7 chương chính và 3 phụ lục kỹ thuật theo chuẩn phân rã công việc (WBS):
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Chương 4}: Thiết kế CSDL cấp độ 1 \& 2 -- Mô hình ý niệm (Conceptual ERD), mô hình quan hệ logic (Logical ERD), chứng minh chuẩn hóa 3NF và từ điển dữ liệu logic.
        \item \textbf{Chương 5}: Thiết kế CSDL cấp độ 3 -- Mô hình vật lý (Physical ERD), thiết kế ràng buộc toàn vẹn, chiến lược đánh chỉ mục hiệu năng, và lập trình CSDL (Views, Stored Procedures, Functions, Triggers).
    \end{itemize}
    \item \textbf{Phần IV -- Kiểm chứng và đảm bảo chất lượng}:
    \begin{itemize}
        \item \textbf{Chương 6}: Bộ ma trận kiểm chứng chất lượng (RTM, CRUD Matrix), kiểm chứng 7 kịch bản nghiệp vụ thực tế (Scenario Walkthrough) và kết quả thực thi 35 kịch bản kiểm thử T-SQL tự động.
    \end{itemize}
    \item \textbf{Phần V -- Tổng kết và hướng phát triển của đề tài}:
    \begin{itemize}
        \item \textbf{Chương 7}: Tổng kết kết quả nghiên cứu và hiện thực hóa đề tài, nhật ký quyết định thiết kế cốt lõi (Design Decision Log), bài học kinh nghiệm, hạn chế thực tế và định hướng phát triển công nghệ trong tương lai.
    \end{itemize}
    \item \textbf{Phụ lục kỹ thuật}:
    \begin{itemize}
        \item \textbf{Phụ lục A}: Kịch bản kiểm thử T-SQL tự động minh chứng khả năng bắt lỗi toàn vẹn của CSDL.
        \item \textbf{Phụ lục B}: Danh mục tra cứu nhanh toàn văn 32 yêu cầu chức năng (FR) và 35 quy tắc nghiệp vụ (BR).
        \item \textbf{Phụ lục C}: Toàn văn mã nguồn DDL cơ sở dữ liệu trên Microsoft SQL Server.
    \end{itemize}
\end{itemize}
