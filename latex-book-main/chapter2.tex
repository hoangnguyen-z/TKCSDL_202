...
\paragraph{Quy ước trạng thái và ranh giới thực thi} Trong chương này, các giá trị trạng thái có dấu gạch dưới như \texttt{PENDING\_VERIFICATION} và \texttt{NEEDS\_CLARIFICATION} là giá trị được lưu trong SQL Server; giao diện có thể hiển thị dưới dạng ``Pending Verification'' hoặc ``Needs Clarification''. Các kiểm tra quyền, phạm vi thao tác và tích hợp dịch vụ ngoài thuộc application/workflow layer; các kiểm tra toàn vẹn dữ liệu, điểm số và giao dịch được thực thi ở SQL Server khi phù hợp.
...
\begin{enumerate}
    ...
    \item \textbf{F04 -- Nộp Bài Dự thi (Film Submission)}: Thí sinh chọn khung hình ở trạng thái \texttt{READY}, thuộc đúng participant, nộp vào hạng mục cuộc thi kèm URI tham chiếu tới tệp ảnh scan và lời tựa nghệ thuật trước thời hạn quy định.
    ...
    \item \textbf{F07 -- Tổng hợp Điểm, Xếp hạng \& Trao giải (Ranking \& Result Finalization)}: Hệ thống tính toán tổng điểm có trọng số và lập danh sách xếp hạng theo hạng mục. Thủ tục finalization tạo kết quả đã khóa; Ban tổ chức thực hiện gán giải và công bố qua các bước workflow riêng.
    \item \textbf{F08 -- Lưu trữ Di sản Số (Digital Archive)}: Sau khi kết quả được finalise, workflow được ủy quyền chụp bản sao lịch sử (\textit{snapshot}) đầy đủ siêu dữ liệu kỹ thuật và kết quả đánh giá để bảo tồn trong kho lưu trữ số. CSDL lưu snapshot và URI tham chiếu; object storage thực tế nằm ngoài scope hiện tại.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{NFR-007 (Xử lý Đồng thời)}: Hệ thống hỗ trợ nhiều cuộc thi diễn ra đồng thời và hàng chục giám khảo chấm điểm đồng thời, bảo toàn tính toàn vẹn giao dịch và giảm thiểu rủi ro xung đột khóa chết (\textit{deadlock}).
    ...
    \item \textbf{NFR-010 (Lưu trữ Hình ảnh Hiệu quả)}: CSDL chỉ lưu trữ chuỗi đường dẫn URI/URL và mã băm SHA-256 khi có dữ liệu băm, không lưu trực tiếp dữ liệu nhị phân ảnh lớn trong bảng giao dịch.
    \item \textbf{NFR-011 (Khả năng lặp lại)}: Quy trình khởi tạo và kiểm thử phải có thể lặp lại cho mục đích đánh giá học thuật và demo.
    \item \textbf{NFR-012 (Tính nhất quán tài liệu)}: Requirement, architecture, database artifact và test phải duy trì cùng hệ mã, status và traceability.
\end{itemize}

\paragraph{Định nghĩa công thức tính điểm có trọng số}
Với điểm thành phần và trọng số phần trăm của từng tiêu chí, tổng điểm của một phiếu chấm được xác định bởi:
\[
\text{Total Score} = \sum_{i=1}^{n}\frac{\text{score}_i \times \text{weight}_i}{100}
\]
Khi nhiều giám khảo chấm cùng một bài ở vòng chung kết, điểm cuối cùng là trung bình của các tổng điểm đã nộp. Trường hợp bằng điểm được xử lý bằng quyết định minh bạch của Ban tổ chức theo chính sách cuộc thi.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Ban tổ chức yêu cầu phát hành cuộc thi. Hệ thống kiểm tra điều kiện toàn vẹn (BR-O-001) và chuyển trạng thái sang \texttt{PUBLISHED}; khi mở đăng ký, trạng thái là \texttt{OPEN}.
    \end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Điều kiện tiên quyết}: Thí sinh có đơn đăng ký \texttt{APPROVED} và eligibility \texttt{ELIGIBLE}; thời gian nộp bài đang mở; khung hình phim ở trạng thái \texttt{READY} và thuộc đúng thí sinh.
    \item \textbf{Luồng sự kiện chính}:
    \begin{enumerate}
        \item Thí sinh chọn cuộc thi, hạng mục và chọn khung hình phim (\textit{Film Frame}) từ cuộn phim của mình.
        \item Lưu URI tham chiếu tới tệp ảnh scan độ phân giải cao và nhập lời tựa nghệ thuật (\textit{Statement}).
        \item Hệ thống kiểm tra thời hạn (BR-O-003) và kiểm tra tính duy nhất của khung hình trong cuộc thi (BR-P-006).
        \item Hệ thống lưu bài thi ở trạng thái \texttt{PENDING\_VERIFICATION} và ghi nhật ký kiểm toán.
    \end{enumerate}
...
\begin{itemize}
    \item \textbf{Tác nhân chính}: Ban tổ chức (Organizer).
    \item \textbf{Điều kiện tiên quyết}: Bài thi đang ở trạng thái \texttt{PENDING\_VERIFICATION} hoặc \texttt{NEEDS\_CLARIFICATION}.
    \item \textbf{Luồng sự kiện chính}:
    \begin{enumerate}
        ...
        \item Ban tổ chức xem xét cảnh báo và đưa ra quyết định: \texttt{VERIFIED}, \texttt{REJECTED} hoặc \texttt{NEEDS\_CLARIFICATION}.
...
\begin{itemize}
    ...
    \item \textbf{Điều kiện tiên quyết}: Giám khảo đã được phân công vào vòng chấm thi; bài thi ở trạng thái \texttt{VERIFIED}; vòng chấm đang trong thời gian hoạt động và yêu cầu nằm trong evaluation window.
    \item \textbf{Luồng sự kiện chính}:
    \begin{enumerate}
    ...
    \item \textbf{Ranh giới thực thi}: Application/workflow layer kiểm tra role, assignment scope và evaluation window; SQL Server kiểm tra score completeness, score range và tính tổng điểm.
\end{itemize}

\subsection{Baseline đầy đủ cho UC-01 đến UC-08}
Để bảo đảm tính truy vết và thống nhất với tám luồng nghiệp vụ, mỗi use case được đối soát theo cùng một bộ trường: trigger, preconditions, main flow, alternative flow, output, status transition, data affected và related business rules.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabular}{p{0.08\linewidth}p{0.13\linewidth}p{0.2\linewidth}p{0.2\linewidth}p{0.22\linewidth}p{0.12\linewidth}}
\toprule
...
\textbf{UC-02} & Participant đăng ký & Account active, registration window mở & Gửi registration, Organizer xét duyệt; trùng hoặc quá hạn thì chặn & Registration; \texttt{PENDING -> APPROVED/REJECTED/WITHDRAWN}; Registration, AuditLog; BR-O-003, BR-P-002, BR-P-003 \\
\textbf{UC-03} & Participant quản lý film asset & ParticipantProfile tồn tại & Tạo roll, frame và metadata; trùng frame hoặc thiếu metadata thì giữ \texttt{DRAFT} & Film asset sẵn sàng; \texttt{DRAFT -> READY -> SUBMITTED/ARCHIVED}; FilmRoll, FilmFrame; BR-O-004, BR-P-004, BR-P-005 \\
\textbf{UC-04} & Participant nộp frame & Registration \texttt{APPROVED}/\texttt{ELIGIBLE}, frame \texttt{READY} thuộc participant, submission window mở & Chọn category, lưu URI scan và statement; quá hạn, sai ownership hoặc trùng frame thì rollback & Submission; \texttt{PENDING\_VERIFICATION}; Submission, AuditLog; BR-O-003, BR-O-005, BR-P-006, BR-P-009 \\
\textbf{UC-05} & Organizer xác minh submission & Submission \texttt{PENDING\_VERIFICATION} hoặc \texttt{NEEDS\_CLARIFICATION} & Kiểm tra metadata, xem AI advisory và quyết định thủ công; thiếu dữ liệu thì yêu cầu clarification & Verification decision; \texttt{PENDING -> UNDER\_REVIEW -> VERIFIED/REJECTED/NEEDS\_CLARIFICATION}; VerificationCase, AIAnalysisResult, Submission; BR-O-006, BR-O-007, BR-P-010, BR-P-011 \\
\textbf{UC-06} & Judge đánh giá & Assignment hợp lệ, submission \texttt{VERIFIED}, round mở & Nhập criterion scores và comment; ngoài workload, trùng evaluation hoặc quá hạn thì chặn & Evaluation; \texttt{DRAFT -> SUBMITTED -> LOCKED}; JudgeAssignment, Evaluation, EvaluationScore, AuditLog; BR-O-008, BR-O-009, BR-P-012, BR-P-013 \\
\textbf{UC-07} & Organizer finalize result & Final round đủ evaluation \texttt{SUBMITTED}/\texttt{LOCKED} & Procedure aggregate/finalize result; workflow xử lý tie, gán award và publish; thiếu evaluation thì chặn & Result; \texttt{FINALIZED -> PUBLISHED}; Result, AwardAssignment, AuditLog; BR-O-010, BR-P-014, BR-P-015, BR-P-016 \\
