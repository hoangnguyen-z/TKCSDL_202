% Chương 4: Thiết kế CSDL Cấp độ 1 & 2: Mô hình Ý niệm và Logic
...
\paragraph{Đồng bộ hóa Tài liệu và Tính Nhất quán Khái niệm}
Bản tài liệu đặc tả kiến trúc và nội dung Chương này cùng thống nhất chia sẻ một hợp đồng ý niệm cốt lõi. Cụ thể, các tên thực thể chuẩn hóa, bản số quan hệ, quy tắc nghiệp vụ và ranh giới quyết định thiết kế được duy trì đồng nhất tuyệt đối trên toàn bộ các tài liệu của dự án; qua đó bản báo cáo mở rộng cấu trúc trình bày học thuật nhưng tuyệt đối không đưa ra một mô hình dị biệt.
...
Trong toàn bộ kiến trúc cơ sở dữ liệu của hệ thống, quan hệ \texttt{ArchiveItem} là trường hợp ngoại lệ duy nhất được cố tình thiết kế theo chiến lược \textbf{Phi chuẩn hóa có kiểm soát (\textit{Controlled Denormalization})}. Cụ thể, thay vì chỉ lưu trữ các khóa ngoại sống (\texttt{result\_id}, \texttt{submission\_id}) tham chiếu động về các bảng giao dịch đang vận hành, thực thể \texttt{ArchiveItem} được cấu trúc hóa để lưu trữ trực tiếp các bản chụp JSON bất biến bao gồm: \texttt{contest\_snapshot}, \texttt{category\_snapshot}, \texttt{participant\_snapshot}, \texttt{technical\_snapshot} và \texttt{judging\_snapshot}.

Quyết định thiết kế này bắt nguồn từ bản chất nghiệp vụ và yêu cầu bảo tồn di sản số trong thế giới thực. Mục tiêu tối thượng của kho lưu trữ di sản không phải là tối ưu hóa thao tác cập nhật thường xuyên, mà là bảo tồn nguyên vẹn giá trị lịch sử và bằng chứng nghệ thuật tại chính xác thời điểm tác phẩm được vinh danh trao giải. Nếu hệ thống áp dụng chuẩn hóa thuần túy bằng việc trỏ khóa ngoại sống, thì trong tương lai khi tài khoản thí sinh thay đổi thông tin cá nhân, hoặc cấu hình cuộc thi gốc bị điều chỉnh, đóng lại hay ẩn đi, dữ liệu của tác phẩm đoạt giải sẽ bị biến dạng theo. Do đó, việc đóng băng toàn bộ ngữ cảnh tại thời điểm trao giải thành các bản chụp JSON độc lập là giải pháp kỹ thuật tối ưu nhất, vừa bảo đảm tính trung thực lịch sử tuyệt đối cho công tác triển lãm và nghiên cứu, vừa ngăn ngừa hoàn toàn các tác động tiêu cực từ các thao tác cập nhật dữ liệu sống trong tương lai.
...
\begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{3.8cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{2.6cm}}
\toprule
...
\scriptsize
\renewcommand{\arraystretch}{0.92}
\begin{tabularx}{\textwidth}{>{\bfseries}p{3.0cm}>{\raggedright\arraybackslash}X}
\toprule
...
Submission & \texttt{DRAFT} $\rightarrow$ \texttt{PENDING\_VERIFICATION} $\rightarrow$ \texttt{VERIFIED} / \texttt{REJECTED} / \texttt{NEEDS\_CLARIFICATION} $\rightarrow$ \texttt{JUDGED} $\rightarrow$ \texttt{FINALIZED} $\rightarrow$ \texttt{ARCHIVED} \\
