% Chương 5: Thiết kế CSDL Cấp độ 3: Mô hình Vật lý và Lập trình CSDL
...
Hệ quản trị cơ sở dữ liệu Microsoft SQL Server (phiên bản 2022+ triển khai trên nền tảng Linux Container) được lựa chọn chính thức cho đề tài dựa trên sự kết hợp của ba căn cứ kỹ thuật then chốt. Trước hết, SQL Server cung cấp cơ chế thực thi ràng buộc toàn vẹn dữ liệu mạnh mẽ hàng đầu hiện nay, hỗ trợ toàn diện các ràng buộc phức tạp như \texttt{CHECK}, \texttt{UNIQUE} và \texttt{FOREIGN KEY} cùng cơ chế khóa giao dịch ACID tuyệt đối, bảo đảm an toàn dữ liệu trong các tình huống chấm thi đồng thời. Mặt khác, ngôn ngữ lập trình T-SQL phong phú cho phép đóng gói logic nghiệp vụ sâu vào các thủ tục lưu trữ (\textit{Stored Procedures}) và \textit{Triggers}, qua đó giảm thiểu độ trễ mạng và ngăn chặn tình trạng thao tác dữ liệu trái phép từ tầng ứng dụng. Bên cạnh đó, khả năng tương thích hoàn hảo và giám sát trực quan thông qua công cụ chuyên nghiệp SQL Server Management Studio (SSMS 22) tạo điều kiện thuận lợi tối đa cho quá trình bảo trì, phân tích kế hoạch thực thi truy vấn (\textit{Execution Plan}) và kiểm thử tự động.
...
Cấu trúc vật lý của toàn bộ 32 bảng cơ sở dữ liệu trên SQL Server được mô tả qua Sơ đồ Vật lý trong Hình~\ref{fig:c5_physical_pdm}:
...
Hệ thống tránh việc sử dụng tùy tiện kiểu \texttt{VARCHAR(255)} cho mọi cột, thay vào đó áp dụng chiến lược lựa chọn kiểu dữ liệu tối ưu cho bộ nhớ và tính chính xác như tổng hợp tại Bảng~\ref{tab:c5_datatypes}:
...
Trong hệ thống quản lý cuộc thi và lưu trữ di sản nghệ thuật, việc bảo toàn toàn vẹn dữ liệu lịch sử và phục vụ kiểm toán là yêu cầu sống còn. Do đó, hệ thống thiết lập nguyên tắc kiến trúc nghiêm ngặt: tuyệt đối không sử dụng hành vi \texttt{ON DELETE CASCADE} đối với bất kỳ bảng giao dịch nghiệp vụ trọng yếu nào (\texttt{Submission}, \texttt{Evaluation}, \texttt{Result}, \texttt{ArchiveItem}). Toàn bộ các liên kết khóa ngoại đều được thiết lập tường minh là \texttt{ON DELETE NO ACTION} (cơ chế bảo vệ mặc định của Microsoft SQL Server) nhằm ngăn chặn hiện tượng vô tình xóa dây chuyền phá hủy chuỗi dữ liệu lịch sử cuộc thi. Mọi hoạt động kết thúc, hủy bỏ hoặc đóng cuộc thi đều được xử lý thông qua cơ chế chuyển đổi trạng thái nghiệp vụ có lưu vết (\textit{Soft Status Transition}), bảo đảm không có bất kỳ bản ghi lịch sử nào bị xóa vật lý khỏi cơ sở dữ liệu.
...
Các chỉ mục phi cụm (\textit{Non-clustered Indexes}) được thiết kế có chọn lọc, gắn liền với các mẫu truy vấn (\textit{Query Access Patterns}) tần suất cao trong thực tế như thể hiện trong Bảng~\ref{tab:c5_indexes}:
