...
\begin{itemize}
    \item \texttt{CK\_contest\_Contest\_dates}: \texttt{registration\_open\_at <= registration\_close\_at AND submission\_open\_at <= submission\_close\_at}.
    \item \texttt{CK\_contest\_ScoringCriterion\_weight}: \texttt{weight\_percent > 0 AND weight\_percent <= 100}.
    \item \texttt{CK\_contest\_ScoringCriterion\_score\_range}: \texttt{score\_min\_value <= score\_max_value}.
    \item \texttt{CK\_verification\_AIAnalysisResult\_confidence}: \texttt{confidence\_score >= 0 AND confidence\_score <= 1}.
    \item \texttt{CK\_judging\_Evaluation\_total\_score}: \texttt{total\_score >= 0}.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{reporting.vw\_judge\_work_queue}: Hàng đợi công việc của từng giám khảo theo vòng chấm.
    ...
    \begin{itemize}
        ...
        \item \texttt{result.usp\_finalize\_results\_for_round}: Tự động tổng hợp điểm, xếp thứ hạng không trùng lặp và khóa kết quả chung cuộc.
    \end{itemize}
